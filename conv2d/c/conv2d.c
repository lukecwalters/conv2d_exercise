...
void conv2d(int N, int Cin, int H, int W, int Kh, int Kw, int stride_h,
            int stride_w, int Cout, int Th, int Tw, float input[N][Cin][H][W],
            float kernel[Cout][Cin][Kh][Kw], float output[N][Cout][Th][Tw],
            float input_2d[N * Tw * Th][Kh * Kw * Cin],
            float kernel_2d[Kh * Kw * Cin][Cout],
            float output_2d[N * Tw * Th][Cout]) {
  // arrange the input tensor as flattened rows of input tiles
  input_to_conv_matrix(N, H, W, Th, Tw, Kh, Kw, Cin, stride_h, stride_w, input,
                       input_2d);
  // flatten the 4D kernel into Cout columns of length Cin*Kw*Kh
  kernel_to_matrix(Cout, Cin, Kh, Kw, kernel, kernel_2d);
  // substitute an optimized matmul routine here in the future for convolution
  // op
  matmul_generic(N * Tw * Th, Kh * Kw * Cin, Cout, input_2d, kernel_2d,
                 output_2d);
  // reshape the matmul result into the expected 4D tensor shape
...
void input_to_conv_matrix(int N, int H, int W, int Th, int Tw, int Kh, int Kw,
                          int Cin, int stride_h, int stride_w,
                          float input[N][Cin][H][W],
                          float input_2d[N * Tw * Th][Kh * Kw * Cin]) {
  ...
  for (int n = 0; n < N; n++) {
    for (int th = 0; th < Th; th++) {
      ...
      for (int tw = 0; tw < Tw; tw++) {
        ...
        row_x++;  // a new row for every example-tile tuple, or for every
                  // (n,th,tw)
      }
    }
  }
...
void matmul_generic(int Ra, int Ca, int Cb, float A[Ra][Ca], float B[Ca][Cb],
                    float Y[Ra][Cb]) {
  float accum = 0.0;
